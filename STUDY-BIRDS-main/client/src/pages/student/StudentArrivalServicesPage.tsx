...
export const StudentArrivalServicesPage = () => {
  ...
  useEffect(() => {
    Promise.all([studentService.getArrivalServices(), studentService.getOverview()])
      ...
      .catch((issue) => setError(getErrorMessage(issue, isArabic ? "تعذر تحميل بيانات الوصول." : "Unable to load arrival services.")));
  }, [isArabic]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setRequest(saved);
      setSuccess(isArabic ? "تم حفظ طلب الوصول والخدمات." : "Arrival services request saved successfully.");
    } catch (issue) {
      setError(getErrorMessage(issue, isArabic ? "تعذر حفظ طلب الوصول." : "Unable to save the arrival request."));
    } finally {
      setSaving(false);
    }
  };
