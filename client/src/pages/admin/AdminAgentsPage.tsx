      ...
      <section className="panel p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">{isArabic ? "استعرض ملف كل وكيل وطلابه وبياناته التشغيلية من مكان واحد." : "Browse each agent profile, students, and operational data in one place."}</p>
          </div>
          ...
          {visibleAgents.map((agent) => {
            ...
            return (
              <button
                ...
                className={`panel w-full p-5 text-left transition ${isActiveAgent ? "ring-2 ring-brand-300" : "hover:-translate-y-0.5"}`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${agent.profile?.verificationStatus === "verified" ? "bg-emerald-100 text-emerald-700" : agent.profile?.verificationStatus === "rejected" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}`}>
                    {agent.profile?.verificationStatus || "pending"}
                  </span>
                </div>
                <div className="mt-4 grid gap-2 text-sm text-slate-600">
                  <p>{agent.profile?.companyName || (isArabic ? "بدون شركة مسجلة" : "No company listed")}</p>
                  <p>{agent.profile?.location || (isArabic ? "بدون موقع" : "No location")}</p>
                  ...
                  {details.verificationDocuments.map((document) => (
                    <div key={document._id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <DocumentFileLink className="inline-flex items-center gap-1 text-brand-700 hover:underline" path={document.filePath}>
